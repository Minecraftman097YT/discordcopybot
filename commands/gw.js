...
module.exports.run = async (Client, message, args, prefix) => { // thats for my cmd handler
    ...
    if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply("Du Hast nicht genug Rechte.");

    // if no time said send this
    if(!args[0]) return message.channel.send(`**Wielange soll das Giveaway gehen?**`)
    ...
    if(isNaN(args[0][0])) return message.channel.send(`**Wielange soll das Giveaway sein?**`)
    ...
    if(!args[1]) return message.channel.send(`**Wieviel Können Gewinnen?**`)
    
    // if no prize said
    if(!args[2]) return message.channel.send(`**Was ist der Preis?**`)
    ...
    var botEmbed = new discord.MessageEmbed()
     ...
     .setDescription(`
     React with 🎉 to enter!
     **Giveaway Preis: **${prize}
     **Giveaway Gewinner: **${winnerCount}
     **Giveaway Endet in: **${args[0]}
    ...
    setTimeout(function () {
        ...
        if(peopleReacted.length == 0) {
            var non = new discord.MessageEmbed()
             ...
             .setDescription(`Kein Gewinner weil niemand Mit gemacht hat :c!
             
              **Giveaway Hosted By: **${message.author}`)
            msg.edit(non)

            return message.channel.send(`Kein Gewinner weil niemand Mit gemacht hat :c! :(\n${msg.url}`)
        }
        ...
        for (let y = 0; y < winners.length; y++) {
            ...
            message.channel.send(`**Glückwunsch an:**\n${response} Du Gewinnst... **${prize}**.\n${msg.url}`) // send a msg with the winner people
        }
...
module.exports.help = {
    name: "giveaway",
    aliases: ["gstart", 'giveaway-start', "gw"]
}
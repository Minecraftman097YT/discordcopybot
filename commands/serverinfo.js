...
module.exports.run = async (Client, message, args, prefix) => { // cmd handler
    ...
    .addField(`General`, [
        ...
        `**Booster:** ${message.guild.premiumTier ? `Tier ${message.guild.premiumTier}` : 'None'}`, // boost tier
        ...
        `**Erstellt:** ${moment(message.guild.createdTimestamp).format('LT')} ${moment(message.guild.createdTimestamp).format('LL')} ${moment(message.guild.createdTimestamp).fromNow()}`, // when did the server got created 
        '\u200b'
    ])
    
    // adding a field with the stats info
    .addField('Stats', [
        `**Rollen:** ${roles.length}`, // how many roles in the server
        `**Emojis:** ${emojis.size}`, // how many emojis
        `**Normale Emojis:** ${emojis.filter(emoji => !emoji.animated).size}`, // how many not animated emojis
        `**Animierte Emojis:** ${emojis.filter(emoji => emoji.animated).size}`, // how many animated emoji
        `**User anzahl:** ${message.guild.memberCount}`, // how many members in the server
        `**User:** ${members.filter(member => !member.user.bot).size}`, // how many are humans
        ...
        `**Bitte nicht Stören:** ${members.filter(member => member.presence.status === 'dnd').size}`, // how many have DND
        `**Abwesend:** ${members.filter(member => member.presence.status === 'idle').size}`, // how many have idle
        `**Text Kanäle:** ${channels.filter(channel => channel.type === 'text').size}`, // how many text channels
        `**Talks:** ${channels.filter(channel => channel.type === 'voice').size}`, // how many voice channels
        '\u200b'
    ])
    
    // adding another field displaying the roles in it
    .addField(`Rolllen [${roles.length - 1}]`, rolesdisplay)
